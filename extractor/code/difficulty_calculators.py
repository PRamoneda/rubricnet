...
from rhythm_chart import song_rhythm_info_from_dict
from raw_data_extractors import load_raw_data, print_warnings
from bayesian_model import keys_distance_colors
...
class KeysDifficultyCalculator(BaseDifficultyCalculator):
    def estimate_difficulty_map(self, raw_data_map, warnings_set):
        ...
        if len(raw_data_map["rh_pitch_sets"]) <= 1:
            result['rh_displacement_stuff'] = 0
        else:
            result['rh_displacement_stuff'] = displacement_stuff(raw_data_map["rh_pitch_sets"])
        if len(raw_data_map["lh_pitch_sets"]) <= 1:
            result['lh_displacement_stuff'] = 0
        else:
...
if __name__ == '__main__':
    ...
    with open(out_file, 'w') as fo:
        ...
        res = []
        for raw_data_map in tqdm(raw_data_maps[0].values()):
            print(raw_data_map['id'])
            warnings_set = set()
            data_path = "../" + raw_data_map['id'] + ".json"
            if os.path.exists(data_path):
                try:
                    with open(data_path, 'r') as f:
                        data = load_raw_data(f)
                    entry = calculator.estimate_difficulty_map(data, warnings_set)
                except Exception as e:
                    print("Error in %s: %s" % (raw_data_map['id'], e))
                    continue
            else:
                continue
            entry['id'] = raw_data_map['id']
            entry['grade'] = raw_data_map['henle']
            res.append(entry)
