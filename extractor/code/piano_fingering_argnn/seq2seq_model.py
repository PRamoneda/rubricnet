...
from piano_fingering_argnn.GGCN import GatedGraph
...
class AR_decoder(nn.Module):
    ...
    def get_lstm_ar_embedding(self, x, fingers):
        # print("teaching forcing")
        fingers = torch.unsqueeze(fingers, dim=2)
        prev_gt = torch.cat(
            (torch.zeros((fingers.shape[0], 1, fingers.shape[2]), device=self.device, dtype=torch.long),
             fingers[:, :-1, :].type(torch.LongTensor).to(self.device)),
            dim=1)
        embed_previous = self.class_embedding(prev_gt)
        concated_data = torch.cat((x, torch.squeeze(embed_previous, dim=2)), dim=2)
        result_lstm, _ = self.ar_lstm(concated_data)
        # result_padded = self.FC(result_lstm)
        # total_result = F.log_softmax(result_padded, dim=2)
        return result_lstm
    ...
    def get_embedding(self, x, x_lengths, edge_list, beam_k=20):
        '''
        ...
        beam_embeddings = torch.zeros((1, 1, self.in_size)).to(self.device)  # prev_out_embedding of each beam case
        cum_prob = torch.ones(1).to(self.device)  # cumulative probability of each beam case
        candidates, candidates_lstm_output, candidates_lstm_hidden = [[]], [[]], [[]]  # decoded fingering sequence of each beam case
        for i in range(x.shape[1]):
            x_repeated = x[:, i:i + 1].repeat(len(beam_embeddings), 1, 1)
            cat_input = torch.cat([x_repeated.to(self.device), beam_embeddings], dim=2)
            out_lstm, hidden_states = self.ar_lstm(cat_input, hidden_states)
            out = self.FC(out_lstm)
            out_prob = F.softmax(out, dim=2)  # use softmax to get probability

            # calculate new cumulative probs for each of new possible cases
            new_cum_prob = cum_prob.unsqueeze(1) * out_prob[:, 0]
            prob_flatten = new_cum_prob.view(-1)
            _, sorted_indices = prob_flatten.sort(descending=True)

            # find the case id with maximum cum_prob with top_k
            selected_beam_ids = sorted_indices[:beam_k]
            cum_prob = prob_flatten[selected_beam_ids]

            # find the parent beam id for each of new selected cases
            prev_beam_indices = torch.div(selected_beam_ids, 5, rounding_mode='floor')
            pred_beam_fingerings = selected_beam_ids % 5

            # Reorder k top sequences of fingerings/embeddings
            # candidates_dasaem = [candidates[prev_beam_indices[i]] + [pred_beam_fingerings[i].item()] for i in
            #                      range(len(selected_beam_ids))]
            new_candidates, new_lstm_output, new_lstm_h = [], [], []
            for j in range(len(selected_beam_ids)):
                prev_beam_index = prev_beam_indices[j]
                new_candidates.append(candidates[prev_beam_index] + [pred_beam_fingerings[j].item()])
                new_lstm_output.append(candidates_lstm_output[prev_beam_index] + [out_lstm[prev_beam_index]])
                new_lstm_h.append(candidates_lstm_output[prev_beam_index] + [hidden_states[0][:, prev_beam_index]])
            candidates, candidates_lstm_output, candidates_lstm_hidden = new_candidates, new_lstm_output, new_lstm_h
            # next hidden
            hidden_states = (hidden_states[0][:, prev_beam_indices],
                             hidden_states[1][:, prev_beam_indices])  # this is for LSTM hidden cell
            beam_embeddings = self.class_embedding(pred_beam_fingerings).unsqueeze(1)
            cum_prob = cum_prob / max(cum_prob)
        # merge all the lstm cell embeddings
        candidates_lstm_output = [torch.cat(candidates_lstm_output[i]).unsqueeze(dim=0) for i in range(5)]
        candidates_lstm_h = [torch.cat(candidates_lstm_hidden[i]).unsqueeze(dim=0) for i in range(5)]
        # check errors
        # for lstm_output, fingers in zip(candidates_lstm_output, candidates):
        #     # check = self.get_lstm_ar_embedding(x, torch.tensor([fingers]))
        #     check = self.FC(lstm_output).argmax(dim=-1)
        #     assert check == fingers, f"{np.array(check) == np.array(fingers)}"
        # create dictionary embeddings
        embedding_save = {
            'encoder': x,
            'lstm_ar_out': candidates_lstm_output,
            'lstm_ar_h': candidates_lstm_h,
            'fingers': candidates
        }
        return candidates, embedding_save
...
class seq2seq(nn.Module):
    ...
    def get_embedding(self, notes, onsets, durations, x_lengths, edge_list, fingers=None, beam_k=10):
        x = self.embedding(notes=notes, onsets=onsets, durations=durations, x_lengths=x_lengths)
        x = self.encoder(x=x, x_lengths=x_lengths, edge_list=edge_list)
        candidates, embedding = self.decoder.get_embedding(x=x, x_lengths=x_lengths, edge_list=edge_list, beam_k=beam_k)
